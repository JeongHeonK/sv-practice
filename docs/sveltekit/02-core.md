...
## 13. Svelte 5 Snippet 패턴

Svelte 5에서 `<slot>`이 deprecated되고 `{#snippet}` / `{@render}`로 대체되었다.
...
컴포넌트 태그 안에서 snippet 블록으로 선언하지 않은 나머지 내용은 자동으로 `children` snippet이 된다.
...
snippet은 파라미터를 가질 수 있으며, 선언된 렉시컬 스코프 안에서만 사용할 수 있다.

### SvelteKit 레이아웃에서의 children prop
...
`+layout.svelte`에서 `children` snippet을 `{@render children()}`으로 렌더링하면 하위 페이지가 레이아웃 안에 삽입된다.
...
`Snippet<[T]>` 타입의 제네릭 인수는 파라미터 타입 튜플이다. 파라미터가 없으면 `Snippet`, 하나면 `Snippet<[T]>`.

### 옵셔널 snippet

```svelte
<!-- optional chaining으로 undefined 처리 -->
{@render children?.()}
